...
public class CreateRoleActivity extends AppCompatActivity {
    ...
    private void setup(){
        ...
        if(!TextUtils.isEmpty(editId)){
            role Entity = realm.where(role.class).equalTo("id", editId).findFirst();
            if(Entity != null)
            {
                isEdit = true;
                EditEntity = Entity;
    ...
    private void createEntity(){
        realm.executeTransactionAsync(new Realm.Transaction() {
            ...
            public void execute(Realm bgRealm) {
                ...
                if(editId != null){
                    newUser = bgRealm.where(role.class).equalTo("id", editId).findFirst();
                } else {
                    newUser = bgRealm.createObject(role.class, UUID.randomUUID().toString());
                }
